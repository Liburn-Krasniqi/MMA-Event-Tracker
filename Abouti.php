<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>About Us</title>
    <link rel="stylesheet" href="style.css">
    <link rel="stylesheet" href="AboutMain.css">
    <link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css">
</head>
<body>
<?php include 'heder.php' ?>

    <main>
        <div class="aboutUs-Container">
            <h1>About Us</h1>
            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Praesentium nulla quo ad placeat a libero dignissimos dolorem! [une edhe redoni jena developerat e qiti websajti te dyt kena probleme psh redoni osht autist, e un duhna mu merr me redonin po apet po ja dalum diqysh :) shpresoj qe historia e jone te i inspiroj te tjeret per me arrit sukses ne jete]   Laborum similique rerum quia incidunt in officia eligendi, repellendus deleniti, molestiae explicabo ea?</p>
        </div>

        <div class="aboutUs-Container">
            <h1>EVEN MORE About Us</h1>
            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Praesentium nulla quo ad placeat a libero dignissimos dolorem! Laborum similique rerum quia incidunt in officia eligendi, repellendus deleniti, molestiae explicabo ea?</p>
        </div>

        <div id="About-title-cards">
            <h1>FAQ</h1>
        </div>

        <div class="faq-container">
            <div class="faq">
                <h3 class="faq-title">
                    A po lodhni hiq?
                </h3>
    
                <p class="faq-text">
                    Hiq valla!
                </p>
    
                <button class="faq-toggle">
                    <i class="fas fa-chevron-down"></i>
                    <i class="fas fa-times"></i>
                </button>
            </div>
        </div>

        <div class="faq-container">
            <div class="faq">
                <h3 class="faq-title">
                    A po ki motiv me vazhdu?
                </h3>
    
                <p class="faq-text">
                    Hiq valla!
                </p>
    
                <button class="faq-toggle">
                    <i class="fas fa-chevron-down"></i>
                    <i class="fas fa-times"></i>
                </button>
            </div>
        </div>

        <div class="faq-container">
            <div class="faq">
                <h3 class="faq-title">
                    Pse?
                </h3>
    
                <p class="faq-text">
                    se di :/
                </p>
    
                <button class="faq-toggle">
                    <i class="fas fa-chevron-down"></i>
                    <i class="fas fa-times"></i>
                </button>
            </div>
        </div>

        <div class="faq-container">
            <div class="faq">
                <h3 class="faq-title">
                    Pse de fol tash?
                </h3>
    
                <p class="faq-text">
                    pfff sdi be kohve tfundit spo lib libi qysh ka libu libi ma perpara U know what im sayin...
                </p>
    
                <button class="faq-toggle">
                    <i class="fas fa-chevron-down"></i>
                    <i class="fas fa-times"></i>
                </button>
            </div>
        </div>
        
    </main>

    <?php include 'hFooter.php' ?>
    <button id="back-to-top-btn"><i class="fa-solid fa-arrow-up"></i></button>
    <script src="app.js"></script> 
    <script src="generate-hed-fot.js" >

    </script>
    
</body>
</html>